...
def gridSearchParameters_SVM_SVC(X, Y, gs_nFolds):
    ...
    {'kernel':['rbf'], 'C':[0.1,1,10,100,1000], 'gamma':[1,0.1,0.01,0.001]},
    {'kernel':['poly'], 'C':[0.1,1,10], 'gamma':[0.1,0.01,0.001], 'degree':[2,3,4]}
    ]
...
def gridSearchParameters_BaggingClassifier(X, Y, gs_nFolds):   
    ...
    param_grid_bg = [
    {'n_estimators':[5,10,15,20,25,30,35,40], 'bootstrap':[True, False], 'bootstrap_features':[True, False]}
    ]
...
def gridSearchParameters_RandomForestClassifier(X, Y, gs_nFolds):   
    ...
    param_grid_rf = [
    {'n_estimators':[10,15,20,25,30,35,40,45], 'criterion':['gini', 'entropy'], 'max_features':['sqrt', 'log2', None], 'bootstrap':[True, False], 'n_jobs':[-1]}
    ]
...
def gridSearchParameters_GradientBoostingClassifier(X, Y, gs_nFolds):   
    ...
    param_grid_gb = [
    {'learning_rate':[0.1, 0.01, 0.001], 'n_estimators':[100,110,120,130,140], 'max_depth':[3,4,5,6]}
    ]
...
def classPiePlot(dataFrameClass):
    ...
    x = []
    for key,val in dictClass.items():
        x.append(val)
    
     
    fig = plt.gcf()
    fig.set_size_inches(3, 3)
    plt.pie(x, labels=['benign', 'malignant'], autopct='%1.2f%%', colors=['lightblue','tan'])
    plt.show()
    #end classPiePlot
...
breastCancerDataFrame = pd.read_csv('Diplomski_ispit/data/BreastCancerWisconsin_DataSet.csv', delimiter=',')
breastCancerDataFrame_NoMissVal = pd.read_csv('Diplomski_ispit/data/BreastCancerWisconsin_DataSet_NoMissVal.csv', delimiter=',')


#Kreiranje korelacione matrice
breastCancerDataFrame_corr = breastCancerDataFrame_NoMissVal[['ClumpThickness', 'UniformityOfCellSize', 'UniformityOfCellShape', 'MarginalAdhesion', 'SingleEpithelialCellSize', 'BareNuclei', 'BlandChromatin', 'NormalNucleoli', 'Mitoses', 'Class']].corr()


#Atribut 'BareNuclei' ima izgubljene vrednosti

print('\nClass distribution, kada su ukljucene vrste sa izgubljenim vrednostima')
classPiePlot(breastCancerDataFrame['Class'])

print('\nClass distribution, kada su izbacene vrste sa izgubljenim vrednostima')
classPiePlot(breastCancerDataFrame['Class'][breastCancerDataFrame.BareNuclei != '?'])


#Postavljanje vrednosti X (predictor) i Y (response)
X = breastCancerDataFrame_NoMissVal[['ClumpThickness', 'UniformityOfCellSize', 'UniformityOfCellShape', 'MarginalAdhesion', 'SingleEpithelialCellSize', 'BareNuclei', 'BlandChromatin', 'NormalNucleoli', 'Mitoses']]
Y = breastCancerDataFrame_NoMissVal['Class']
...
for c in [0.1,0.01,0.001]:
    print('\nl1 kada je C=', c)
    svm_l1 = LinearSVC(penalty='l1', C=c, dual=False)
    crossValidationScore(X, Y, {}, 10, ('svm_l1', svm_l1) )
...
print('\nIzabran je l1 SVM sa parametrom C=0.01')
svm_l1 = LinearSVC(penalty='l1', C=0.01, dual=False)
svm_l1.fit(X, Y)
...
ranking = pd.concat( [ ranking, pd.Series(l1 , name = 'L1-based') ], axis=1  )
...
print('\n\nSelektovani su atributi: \'UniformityOfCellSize\' i \'BareNuclei\'\n ')
X_FS = breastCancerDataFrame_NoMissVal[['UniformityOfCellSize', 'BareNuclei']]
