...
def gridSearchParameters_SVM_L1(X, Y, gs_nFolds):
    ...
    param_grid_svm = [
    {'penalty':['l1'], 'C':[1, 0.1, 0.01, 0.001], 'dual':[False]}
    ]
...
def gridSearchParameters_BaggingClassifier(X, Y, gs_nFolds):   
    ...
    grid = grid_search.GridSearchCV(BaggingClassifier(base_estimator=dt), param_grid_bg, scoring='accuracy', n_jobs=-1, cv=gs_nFolds)
    grid.fit(X, Y)
...
def gridSearchParameters_GradientBoostingClassifier(X, Y, gs_nFolds):   
    ...
    param_grid_gb = [
    {'learning_rate':[0.1, 0.05, 0.025], 'n_estimators':[80,90,100,110,120], 'max_depth':[1,2,3,4]}
    ]
...
def classBarhPlot(dataFrameClass):
    #Crta barh plot kojim se predstavlja procentualni odnos instanca po klasi 
    dictClass = {}
    i=1
    while i <= 10:
        dictClass[i] = 0
    ...
    class_series = pd.Series(x, ['A','B','C','D','SH','AD','DE','LD','FS','SUSP'])
...
#Ucitavanje podataka
cardiotocographyData = pd.read_csv('Diplomski_ispit/data/Cardiotocography.csv', delimiter=',')

#Postavljanje vrednosti X (predictor)
X = cardiotocographyData[['LB', 'AC', 'FM', 'UC', 'DL', 'DS', 'DP', 'ASTV', 'MSTV', 'ALTV', 'MLTV', 'Width', 'Min', 'Max', 'Nmax', 'Nzeros', 'Mode', 'Mean','Median', 'Variance', 'Tendency']]


################################################################################
#Obrada kada Y ima CLASS vrednost

print('Y ima CLASS vrednost')

Y_CLASS = cardiotocographyData['CLASS']

#Odnos broja instanci za CLASS
print('\nOdnos broja instanci za CLASS')
classBarhPlot(Y_CLASS)
...
#K Nearest Neighbors
kNeighbors = gridSearchParameters_KNeighborsClassifier(X, Y_CLASS, gs_nFolds)

#Support Vector Machines
svm_svc = gridSearchParameters_SVM_SVC(X, Y_CLASS, gs_nFolds)


#Ensemble Methods

#Bagging
bagging = gridSearchParameters_BaggingClassifier(X, Y_CLASS, gs_nFolds)

#Random Forests
randomForest = gridSearchParameters_RandomForestClassifier(X, Y_CLASS, gs_nFolds)

#Gradient Boosting 
gradientBoosting = gridSearchParameters_GradientBoostingClassifier(X, Y_CLASS, gs_nFolds)
#gradientBoosting = GradientBoostingClassifier()


scoreAll = {}
cv_nFolds = 10
crossValidationScore(X, Y_CLASS, scoreAll, cv_nFolds, ('NaiveBayesG', naiveBayesG), ('k-NN', kNeighbors), ('SVM', svm_svc), ('Bagging', bagging), ('RandomForests', randomForest), ('GradientBoosting', gradientBoosting) )
...
cardiotocographyData_corr_CLASS = cardiotocographyData[['LB', 'AC', 'FM', 'UC', 'DL', 'DS', 'DP', 'ASTV', 'MSTV', 'ALTV', 'MLTV', 'Width', 'Min', 'Max', 'Nmax', 'Nzeros', 'Mode', 'Mean','Median', 'Variance', 'Tendency', 'CLASS']].corr()


#Dataframe u kome se nalaze rezultati metoda za selekciju atributa
ranking_CLASS = pd.DataFrame(X.columns.values, columns=['columns'])
...
gradientBoosting.fit(X,Y_CLASS)
...
ranking_CLASS = pd.concat( [ ranking_CLASS, pd.Series( sortScore(gradientBoosting_score ) , name = 'gradientBoosting') ], axis=1  )
...
selectK_chi2.fit(X_bin,Y_CLASS)
...
ranking_CLASS = pd.concat( [ ranking_CLASS, pd.Series(sortScore(selectK_chi2.scores_) , name = 'chi2') ], axis=1  )
...
selectK_f_classif.fit(X,Y_CLASS)
...
ranking_CLASS = pd.concat( [ ranking_CLASS, pd.Series(sortScore(selectK_f_classif.scores_) , name = 'F-value') ], axis=1  )
...
svm_svc_linear =  gridSearchParameters_SVM_Linear(X, Y_CLASS, gs_nFolds)
crossValidationScore(X, Y_CLASS, {}, 10, ('svm_svc_linear', svm_svc_linear) )
...
ranking_CLASS = pd.concat( [ ranking_CLASS, pd.Series(rfe.ranking_ , name = 'RFE') ], axis=1  )
...
svm_l1 = gridSearchParameters_SVM_L1(X, Y_CLASS, gs_nFolds)
crossValidationScore(X, Y_CLASS, {}, 10, ('svm_l1', svm_l1) )

svm_l1.fit(X, Y_CLASS)
model = SelectFromModel(svm_l1, prefit=True)
l1 = model.get_support()
print('\nl1 feature selection:', l1)

ranking_CLASS = pd.concat( [ ranking_CLASS, pd.Series(l1 , name = 'L1-based') ], axis=1  )
...
X_FS = cardiotocographyData[[ 'AC',  'UC', 'DL',  'DP', 'ASTV', 'MSTV', 'ALTV', 'MLTV', 'Width',  'Max',  'Mean', 'Variance']]


print('\nSelektovani atributi: AC, UC, DL, DP, ASTV, MSTV, ALTV, MLTV, Width, Max, Mean, Variance')
...
#K Nearest Neighbors
kNeighbors = gridSearchParameters_KNeighborsClassifier(X_FS, Y_CLASS, gs_nFolds)

#Support Vector Machines
svm_svc = gridSearchParameters_SVM_SVC(X_FS, Y_CLASS, gs_nFolds)


#Ensemble Methods

#Bagging
bagging = gridSearchParameters_BaggingClassifier(X_FS, Y_CLASS, gs_nFolds)

#Random Forests
randomForest = gridSearchParameters_RandomForestClassifier(X_FS, Y_CLASS, gs_nFolds)

#Gradient Boosting 
gradientBoosting = gridSearchParameters_GradientBoostingClassifier(X_FS, Y_CLASS, gs_nFolds)
#gradientBoosting = GradientBoostingClassifier()


scoreFS1 = {}
cv_nFolds = 10
crossValidationScore(X_FS, Y_CLASS, scoreFS1, cv_nFolds, ('NaiveBayesG', naiveBayesG), ('k-NN', kNeighbors), ('SVM', svm_svc), ('Bagging', bagging), ('RandomForests', randomForest), ('GradientBoosting', gradientBoosting) )
crossValidationBoxplot(scoreFS1, 10, 3, 'NaiveBayesG', 'k-NN', 'SVM', 'Bagging', 'RandomForests', 'GradientBoosting')
...
crossValidationCompareBoxplot(scoreAll, scoreFS1, 'Svi atributi', 'Selektovani atributi', 10, 3.1, 'NaiveBayesG', 'k-NN', 'SVM')

print('\nUporedne vrednosti za Ensemble metode')
crossValidationCompareBoxplot(scoreAll, scoreFS1, 'Svi atributi', 'Selektovani atributi', 10, 3.1, 'Bagging', 'RandomForests', 'GradientBoosting')



#Analiza sa selektovanim atributima

X_FS = cardiotocographyData[[ 'AC',  'DL',  'DP', 'ASTV', 'MSTV', 'ALTV',  'Width',   'Mean', 'Variance']]


print('\nSelektovani atributi: AC, DL, DP, ASTV, MSTV, ALTV, Width, Mean, Variance')
...
scoreFS2 = {}
cv_nFolds = 10
crossValidationScore(X_FS, Y_CLASS, scoreFS2, cv_nFolds, ('NaiveBayesG', naiveBayesG), ('k-NN', kNeighbors), ('SVM', svm_svc), ('Bagging', bagging), ('RandomForests', randomForest), ('GradientBoosting', gradientBoosting) )
crossValidationBoxplot(scoreFS2, 10, 3, 'NaiveBayesG', 'k-NN', 'SVM', 'Bagging', 'RandomForests', 'GradientBoosting')
...
print('\nUporedne vrednosti za Naive Bayes, K Nearest Neighbors i SVM')
crossValidationCompareBoxplot(scoreAll, scoreFS2, 'Svi atributi', 'Selektovani atributi', 10, 3.1, 'NaiveBayesG', 'k-NN', 'SVM')

print('\nUporedne vrednosti za Ensemble metode')
crossValidationCompareBoxplot(scoreAll, scoreFS2, 'Svi atributi', 'Selektovani atributi', 10, 3.1, 'Bagging', 'RandomForests', 'GradientBoosting')



########################################
#Uporedne vrednosti za obe selekcija atributa

print('\nUporedne vrednosti nakon obe selekcije atributa')

print('\nUporedne vrednosti za Naive Bayes, K Nearest Neighbors i SVM')
crossValidationCompareBoxplot(scoreFS1, scoreFS2, 'Prva selekcija', 'Druga selekcija', 10, 3.1, 'NaiveBayesG', 'k-NN', 'SVM')

print('\nUporedne vrednosti za Ensemble metode')
crossValidationCompareBoxplot(scoreFS1, scoreFS2, 'Prva selekcija', 'Druga selekcija', 10, 3.1, 'Bagging', 'RandomForests', 'GradientBoosting')
