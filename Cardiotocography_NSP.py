...
def gridSearchParameters_BaggingClassifier(X, Y, gs_nFolds):   
    ...
    param_grid_bg = [
    {'n_estimators':[5,10,15,20,25,30,35,40,45,50], 'bootstrap':[True, False], 'bootstrap_features':[True, False]}
    ]
...
def gridSearchParameters_RandomForestClassifier(X, Y, gs_nFolds):   
    ...
    param_grid_rf = [
    {'n_estimators':[5,10,15,20,25,30,35,40,45,50], 'criterion':['gini', 'entropy'], 'max_features':['sqrt', 'log2', None], 'bootstrap':[True, False], 'n_jobs':[-1]}
    ]
...
def gridSearchParameters_GradientBoostingClassifier(X, Y, gs_nFolds):   
    ...
    param_grid_gb = [
    {'learning_rate':[0.1, 0.05, 0.025, 0.0125], 'n_estimators':[40,50,60,70,80], 'max_depth':[2,3,4]}
    ]
...
def classPiePlot(dataFrameClass):
    ...
    plt.pie(x,labels=['normal','suspect','pathologic'] , autopct='%1.2f%%', colors=['lightblue','tan','gray'])
    plt.show()
    # end classPiePlot
...
################################################################################
#Obrada kada Y ima NSP vrednost

print('\nY ima NSP vrednost')

Y_NSP = cardiotocographyData['NSP']

#Procentualni odnos instanca po klasi
print('\nProcentualni odnos instanca za NSP')
classPiePlot(Y_NSP)
...
#K Nearest Neighbors
kNeighbors = gridSearchParameters_KNeighborsClassifier(X, Y_NSP, gs_nFolds)

#Support Vector Machines
svm_svc = gridSearchParameters_SVM_SVC(X, Y_NSP, gs_nFolds)


#Ensemble Methods

#Bagging
bagging = gridSearchParameters_BaggingClassifier(X, Y_NSP, gs_nFolds)

#Random Forests
randomForest = gridSearchParameters_RandomForestClassifier(X, Y_NSP, gs_nFolds)

#Gradient Boosting 
gradientBoosting = gridSearchParameters_GradientBoostingClassifier(X, Y_NSP, gs_nFolds)
#gradientBoosting = GradientBoostingClassifier()


scoreAll = {}
cv_nFolds = 10
crossValidationScore(X, Y_NSP, scoreAll, cv_nFolds, ('NaiveBayesG', naiveBayesG), ('k-NN', kNeighbors), ('SVM', svm_svc), ('Bagging', bagging), ('RandomForests', randomForest), ('GradientBoosting', gradientBoosting) )
...
cardiotocographyData_corr_NSP = cardiotocographyData[['LB', 'AC', 'FM', 'UC', 'DL', 'DS', 'DP', 'ASTV', 'MSTV', 'ALTV', 'MLTV', 'Width', 'Min', 'Max', 'Nmax', 'Nzeros', 'Mode', 'Mean','Median', 'Variance', 'Tendency', 'NSP']].corr()


#Dataframe u kome se nalaze rezultati metoda za selekciju atributa
ranking_NSP = pd.DataFrame(X.columns.values, columns=['columns'])
...
ranking_NSP = pd.concat( [ ranking_NSP, pd.Series( sortScore(gradientBoosting_score ) , name = 'gradientBoosting') ], axis=1  )
...
ranking_NSP = pd.concat( [ ranking_NSP, pd.Series(sortScore(selectK_chi2.scores_) , name = 'chi2') ], axis=1  )
...
ranking_NSP = pd.concat( [ ranking_NSP, pd.Series(sortScore(selectK_f_classif.scores_) , name = 'F-value') ], axis=1  )
...
svm_svc_linear =  gridSearchParameters_SVM_Linear(X, Y_NSP, gs_nFolds)
crossValidationScore(X, Y_NSP, {}, 10, ('svm_svc_linear', svm_svc_linear) )
...
ranking_NSP = pd.concat( [ ranking_NSP, pd.Series(rfe.ranking_ , name = 'RFE') ], axis=1  )
...
svm_l1 = gridSearchParameters_SVM_L1(X, Y_NSP, gs_nFolds)
crossValidationScore(X, Y_NSP, {}, 10, ('svm_l1', svm_l1) )
...
ranking_NSP = pd.concat( [ ranking_NSP, pd.Series(l1 , name = 'L1-based') ], axis=1  )
...
print('\n\nSelektovani su atributi: \'DP\', \'ASTV\' i \'ALTV\' \n ')

X_FS = cardiotocographyData[['DP', 'ASTV', 'ALTV']]
...
#K Nearest Neighbors
kNeighbors = gridSearchParameters_KNeighborsClassifier(X_FS, Y_NSP, gs_nFolds)

#Support Vector Machines
svm_svc = gridSearchParameters_SVM_SVC(X_FS, Y_NSP, gs_nFolds)


#Ensemble Methods

#Bagging
bagging = gridSearchParameters_BaggingClassifier(X_FS, Y_NSP, gs_nFolds)

#Random Forests
randomForest = gridSearchParameters_RandomForestClassifier(X_FS, Y_NSP, gs_nFolds)

#Gradient Boosting 
gradientBoosting = gridSearchParameters_GradientBoostingClassifier(X_FS, Y_NSP, gs_nFolds)
#gradientBoosting = GradientBoostingClassifier()


scoreFS = {}
cv_nFolds = 10
crossValidationScore(X_FS, Y_NSP, scoreFS, cv_nFolds, ('NaiveBayesG', naiveBayesG), ('k-NN', kNeighbors), ('SVM', svm_svc), ('Bagging', bagging), ('RandomForests', randomForest), ('GradientBoosting', gradientBoosting) )
